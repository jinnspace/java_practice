...
public class ControlOp_While01 {
	...
	/* while 반복문
	 * for 반복문은 정해진 횟수의 반복을 괄호 안에 미리 정의 해놓고 반복하는 특징이 있습니다.
	 * while 문은 정해지지 않은 횟수의 반복을 처리하는데 주로 사용합니다.
	 * 특정 조건이 만족할때까지 무한 수행합니다.
	 * 
	 * while( true ) {} -> 이 반복문은 종료되지 않습니다. : 무한반복 
	 */
     
		
		// for 문을 이용해서 10회 반복 실행
		for(int i = 1; i<=10; i++) {
			System.out.println("for : " + i);
		}
		System.out.println();
		// i <= 10 : 반복을 계속할 조건. 이 관계연산이 false이면 반복실행을 멈춥니다.
		...
		System.out.println();
		// 괄호 안의 관계(비교) 연산식의 참/거짓 유무를 먼저 판단한 후 반복을 계속할지 결정합니다.
		// while문의 경우 위에서 코딩한것 처럼 a++ 명령에 의해 반복 유지 유무가 결정되는 것보단
		// 특정 조건에 의해 순서와 반복 횟수에 상관없이 반복이 결정되는 상황에 더 적절하게 사용됩니다. 
